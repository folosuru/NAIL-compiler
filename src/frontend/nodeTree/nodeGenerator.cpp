...
#include "NodeIncludeList.hpp"
#include "frontend/Printer/ErrorPrinter.hpp"
#include "frontend/Symbol/scope/GlobalScope.hpp"
#include "frontend/nodeTree/syntax/BlockNode.hpp"
#include "frontend/tokenize/TokenUtility.hpp"
...
NodeType primary(const std::shared_ptr<TokenList>& list, const std::shared_ptr<Scope>& scope) {
    ...
    if (auto token = TokenUtil::consume_current<Token::PreservedSymbol>(list)) {
        if (token->getSymbolType() == Token::PreservedSymbol::Symbol_type::kw_var) {
            ...
            return std::make_shared<IdentifyNode>(scope,
                                                  std::string(name->getString()),
                                                  name); // これ多分、変数のnodeじゃなくてただの変数のNodeで十分だね。
        }
    }
    ...
    while (true) {
        ...
        if (list->consume_current(".")) {
            result =
                BinaryTree::create(BinaryTree::Type::member_access, scope, result, function_call(list, scope), current);
            continue;
        }
        return result;
    }
