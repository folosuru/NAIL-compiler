...
namespace NAIL_cl::ErrorPrinter {
    void print(const std::shared_ptr<TokenList>& list,
        ...
        std::cout << std::setw( token->pos+1 + 4 + 4 + static_cast<int>(std::log10(token->pos+1))+1) << "^"
         <<  std::setfill('~') << std::setw(token->getString().size() - 1) << "" << std::setfill(' ') << "\n";
        if (suggest) {
            std::cout << std::setw( token->pos+1 + 4 + 4 + static_cast<int>(std::log10(token->pos+1)) +1) << suggest.value() << "\n";
        }
        exit(0);
    }
